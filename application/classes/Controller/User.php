...
class Controller_User extends Controller_PageTemplate {
    ...
    public function action_logged() {
        if ($this->_auth->logged_in())
            $this->template->content = View::factory('user/success-logged');
        else
            $this->_unlogged();
    }
    ...
    public function action_loggedin() {
        if ($this->_auth->logged_in())
            $this->template->content = View::factory('user/loggedin');
        else
            $this->_unlogged();
    }
    ...
    public function action_myprofile() {
        if ($this->_auth->logged_in()) {
            $this->template->content = View::factory('user/profile/info');
        } else {
    ...
    public function action_edit() {
        if ($this->_auth->logged_in()) {
            $this->template->content = View::factory('user/profile/edit');
        } else {
            //przekierowanie na brak dostępu lub na logowanie 
        }
    }
    ...
    public function action_changepassword() {
        if ($this->_auth->logged_in()) {
            $this->template->content = View::factory('user/profile/changepassword');
        } else {
            //przekierowanie na brak dostępu lub na logowanie 
        }
    }
    ...
    /*
     * obłsguje wyświetlanie informacji o logowaniu
     */

    public function action_loginsinfo() {
        if ($this->_auth->logged_in()) {
            $this->template->content = View::factory('user/profile/loginsinfo');
        } else {
